...
@transform([assembleWithStringTie, mergeAllAssemblies, merge_by_tissue],
           suffix(".gtf.gz"),
           add_inputs(PARAMS["annotations_filtered_geneset"]),
           ".class.gz")
...
#@follows(find_utrons, mergeAllAssemblies, mergeAllQuants)
@transform(PARAMS["annotations_filtered_geneset"], regex("(.+).gtf.gz"), "expression.dir/gtf_stop_codons.txt")
def gtf_stop_codons(infile, gtf):
    outfile = open(gtf, "w")
    for line in DataIterator(infile):
        if line.featuretype == "stop_codon":
            if line.strand == "+":
                outfile.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (''.join(line.attributes['gene_id']),''.join(line.attributes['transcript_id']), line.strand, line.featuretype, line.seqid, line.start, line.end))
            elif line.strand =="-":
                outfile.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (''.join(line.attributes['gene_id']), ''.join(line.attributes['transcript_id']), line.strand, line.featuretype, line.seqid, line.end, line.start))
        else:
            pass
    outfile.close()
